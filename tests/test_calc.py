...
@pytest.mark.parametrize(
    "filename",
    [
        os.path.join("tests", "SSX70065.IDE"),
        os.path.join(".", "tests", "test1.IDE"),
        os.path.join(".", "tests", "test2.IDE"),
    ],
)
def test_make_peak_windows(filename):
    with idelib.importFile(filename) as ds:
        ...
        calc_meta = bsvp.calc._make_meta(ds)
        calc_peaks = bsvp.calc._make_peak_windows(analyzer, margin_len=10)
...
def data_builder():
    return (
        bsvp.calc.GetDataBuilder(accel_highpass_cutoff=1)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        os.path.join(".", "tests", "test4.IDE"),
        os.path.join(".", "tests", "test5.IDE"),
        os.path.join(".", "tests", "GPS-Chick-Fil-A_003.IDE"),
        # os.path.join(".", "tests", "test_GPS_2.IDE"),
        # os.path.join(".", "tests", "test_GPS_3.IDE"),
    ],
)
...
def assert_output_is_valid(output: bsvp.calc.OutputStruct):
    """Validate the contents & structure of an `OutputStruct` object."""
    assert isinstance(output, bsvp.calc.OutputStruct)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        bsvp.calc.GetDataBuilder(accel_highpass_cutoff=1),
        bsvp.calc.GetDataBuilder(accel_highpass_cutoff=1).add_psd(freq_bin_width=1),
        bsvp.calc.GetDataBuilder(accel_highpass_cutoff=1).add_pvss(
            init_freq=1, bins_per_octave=12
        ),
        bsvp.calc.GetDataBuilder(accel_highpass_cutoff=1).add_metrics(),
        bsvp.calc.GetDataBuilder(accel_highpass_cutoff=1).add_peaks(margin_len=1000),
        bsvp.calc.GetDataBuilder(accel_highpass_cutoff=1).add_vc_curves(
        ...
        bsvp.calc.GetDataBuilder(accel_highpass_cutoff=None).add_psd(freq_bin_width=1),
        # Test time restrictions on acceleration data
        bsvp.calc.GetDataBuilder(
        ...
        bsvp.calc.GetDataBuilder(accel_highpass_cutoff=1).add_psd(bins_per_octave=1),
        bsvp.calc.GetDataBuilder(accel_highpass_cutoff=1).add_psd(
...
def output_struct():
    ...
    result = bsvp.calc.OutputStruct(data)
